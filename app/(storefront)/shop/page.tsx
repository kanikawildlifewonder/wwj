...
import { MOCK_PRODUCTS, PRODUCT_CATEGORIES, ANIMAL_INSPIRATIONS } from "@/lib/mock-data";
...
export default function ShopPage() {
  ...
  const FilterPanel = () => (
    <div className="space-y-8">
      ...
      {/* Animal Inspiration */}
      <div>
        <h3 className="font-sans text-xs tracking-widest uppercase font-bold text-jungle mb-4">Animal Inspiration</h3>
        <div className="space-y-2">
          {ANIMAL_INSPIRATIONS.map((animal) => (
            <button
              key={animal}
              onClick={() => setSelectedAnimal(animal)}
              className={`block text-sm w-full text-left py-1 transition-colors ${selectedAnimal === animal ? "text-gold-dark font-bold" : "text-jungle/70 hover:text-jungle"}`}
            >
              {animal}
            </button>
          ))}
