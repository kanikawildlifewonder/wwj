...
export default function EditEventForm({ event }: { event: SerializedEvent }) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [products, setProducts] = useState<ProductOption[]>([]);
  const [productSearch, setProductSearch] = useState("");

  // Convert ISO string date to datetime-local input format (YYYY-MM-DDThh:mm)
  ...
  const [formData, setFormData] = useState({
    ...
    eventDate: formatIsoToInput(event.eventDate),
  ...
  const [selectedProductIds, setSelectedProductIds] = useState<string[]>(event.relatedProducts || []);

  // Media files previews & updates
  const [mainImageFile, setMainImageFile] = useState<File | null>(null);
  const [mainImagePreview, setMainImagePreview] = useState<string | null>(event.featuredImage || null);
  ...
  const [galleryPreviews, setGalleryPreviews] = useState<(string | null)[]>([
    event.galleryImages[0] || null,
    event.galleryImages[1] || null,
    event.galleryImages[2] || null,
    event.galleryImages[3] || null,
  ]);

  const [partnerLogoFile, setPartnerLogoFile] = useState<File | null>(null);
  const [partnerLogoPreview, setPartnerLogoPreview] = useState<string | null>(event.partnerLogo || null);
  ...
  const removePartnerLogo = (e: React.MouseEvent) => {
    e.preventDefault();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.slug || !formData.eventDate || !mainImagePreview) {
      toast.error("Please fill in all required fields (Title, Slug, Event Date, and Main Image).");
      return;
    }

    setIsSubmitting(true);
    let mainImageUrl = event.featuredImage;
    const finalGalleryUrls = [...galleryPreviews];
    let partnerLogoUrl: string | null = partnerLogoPreview;

    try {
      toast.loading("Uploading new media files...", { id: "upload" });
      ...
      // Partner logo upload if changed
      if (partnerLogoFile) {
        uploadPromises.push(
      ...
      if (uploadPromises.length > 0) {
        ...
        for (const result of results) {
          ...
          else if (result.type === "gallery") finalGalleryUrls[result.index] = result.res.url;
          else if (result.type === "partner") partnerLogoUrl = result.res.url;
        }
        toast.success("Media uploads synchronized!", { id: "upload" });
      } else {
        toast.dismiss("upload");
      }

      // If partnerLogoPreview was deleted, set database field to null
      if (!partnerLogoPreview) {
        partnerLogoUrl = null;
      }

      // 2. Update Event
      toast.loading("Saving changes to event...", { id: "save" });
      const res = await updateEvent(event.id, {
        ...
        galleryImages: finalGalleryUrls.filter(Boolean) as string[],
      ...
      if (res.success) {
        ...
        toast.error(res.error || "Failed to update event", { id: "save" });
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="font-display text-2xl text-jungle">Edit Event / Collaboration</h2>
          <p className="text-sm text-jungle/60">Update details, media gallery, and relations for this story.</p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-border p-6 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {partnerLogoPreview ? (
                  <div className="relative flex items-center justify-center w-full h-24 border border-border rounded-xl bg-cream/30 group">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 border border-border rounded-xl bg-cream/40 hover:bg-cream/60 focus:bg-cream/80 text-jungle focus:outline-none focus:border-gold transition-all"
                >
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        className="rounded border-border text-gold focus:ring-gold w-3.5 h-3.5 bg-cream/40 hover:bg-cream/60 focus:bg-cream/80"
                      />
                      <div
